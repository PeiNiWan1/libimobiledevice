...
#define OPT_FULL 9
	...
	checkpoint:
		...
		case CMD_ERASE:
		{
			PRINT_VERBOSE(1, "Sending EraseDevice request...\n");
			plist_t cmd_plist = plist_new_dict();
			plist_dict_set_item(cmd_plist, "ForceFullBackup", plist_new_bool(0));
			err = mobilebackup2_send_request(mobilebackup2, "EraseDevice", udid, source_udid, cmd_plist);
			if (err != MOBILEBACKUP2_E_SUCCESS)
			{
				printf("Could not send EraseDevice request, error code %d\n", err);
			}
			else
			{
				PRINT_VERBOSE(1, "EraseDevice request sent successfully.\n");
			}
			plist_free(cmd_plist);
